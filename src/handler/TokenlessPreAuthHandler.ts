/**
 *  Tokenless PreAuth Handler
 */
import { captureLambdaHandler, Tracer } from "@aws-lambda-powertools/tracer";
import {
  BUILDER_API_GATEWAY_MIDDLEWARE,
  ContentTypeEnum,
  DYNAMO_PARAMS_MIDDLEWARE,
...
import { INIT_CARD_TOKEN_MIDDLEWARE } from "@kushki/core/lib/middleware/InitCardTokenMiddleware";
...
> = core<IAPIGatewayEvent<string | object>>(
  ROLLBAR.lambdaHandler(
    ...
    >(
      ...
      "tokenlessPreAuthorization", // Service Method
      CONTAINER,
      ROLLBAR
    )
  )
  ...
  .use(DYNAMO_PARAMS_MIDDLEWARE(ROLLBAR, CONTAINER, false))
  .use(
    BUILDER_API_GATEWAY_MIDDLEWARE(
      ROLLBAR,
  ...
  .use(
    VALIDATION_MIDDLEWARE(ROLLBAR, {
      body: {
        name: SchemaEnum.tokenless_charge_request,
      },
      headerForValidation: "KSH-Authorization",
    })
  )
  .use(INIT_CARD_TOKEN_MIDDLEWARE(CONTAINER, ROLLBAR))
